...
const ResultDetailsModal: React.FC<ResultDetailsModalProps> = ({ isOpen, onClose, result }) => {
  ...
  const getStatusColor = (isVerified: boolean) => {
    return isVerified ? 'text-success-600 bg-success-100' : 'text-warning-600 bg-warning-100';
  };

  const getStatusIcon = (isVerified: boolean) => {
    return isVerified ? <CheckCircleIcon className='h-4 w-4' /> : <ClockIcon className='h-4 w-4' />;
  };
        ...
        <div className='bg-gray-50 rounded-lg p-4'>
          <div className='flex items-start justify-between'>
            ...
            <div className='flex flex-col items-end space-y-2'>
              <span
                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(result.isVerified)}`}
              >
                    ...
                    <div className='text-right'>
                      ...
                      <p className='text-xs text-gray-500'>
                        {calculateTotalVotes() > 0
                          ? ((candidate.votes / calculateTotalVotes()) * 100).toFixed(1)
                          : 0}
                        %
                      </p>
                    </div>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
              ...
              <div className='text-center'>
                <p className='text-2xl font-bold text-green-600'>
                  {result.voteData.validVotes || 0}
                </p>
                <p className='text-sm text-gray-600'>Valid Votes</p>
              </div>
              <div className='text-center'>
                <p className='text-2xl font-bold text-red-600'>
                  {result.voteData.invalidVotes || 0}
                </p>
