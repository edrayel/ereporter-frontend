...
const Landing: React.FC = () => {
  ...
  const generateMockResults = (): LiveResult[] => {
    ...
    const stateCoordinates: { [key: string]: { lat: number; lng: number } } = {
      ...
      'Cross River': { lat: 5.9630, lng: 8.3405 },
      'Ebonyi': { lat: 6.2649, lng: 8.0137 },
      'Ekiti': { lat: 7.7193, lng: 5.3110 },
      'Enugu': { lat: 6.5244, lng: 7.5086 },
      'FCT': { lat: 9.0765, lng: 7.3986 },
      'Gombe': { lat: 10.2904, lng: 11.1670 },
      'Jigawa': { lat: 12.2300, lng: 9.3500 },
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const stateCoord = stateCoordinates[state] || { lat: 9.0820, lng: 8.6753 }; // Default to Nigeria center
      const totalVotes = Math.floor(Math.random() * 5000) + 500;
      const candidateResults = parties.map(party => {
      ...
      results.push({
        ...
        status: ['verified', 'pending', 'disputed'][Math.floor(Math.random() * 3)] as 'verified' | 'pending' | 'disputed',
  ...
  const filteredResults = selectedState === 'all' 
    ? liveResults 
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <nav className="hidden md:ml-8 md:flex md:space-x-8">
                ...
                <a href="#live" className="text-gray-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors">
                  Live Results
                </a>
      ...
      <section className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-primary-600">{stats.totalPollingUnits.toLocaleString()}</div>
              <div className="text-sm text-gray-500 mt-1">Polling Units</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-green-600">{stats.reportedResults.toLocaleString()}</div>
              <div className="text-sm text-gray-500 mt-1">Results Reported</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600">{stats.verifiedResults.toLocaleString()}</div>
              <div className="text-sm text-gray-500 mt-1">Verified Results</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600">{stats.totalVotes.toLocaleString()}</div>
                    ...
                    <div key={result.id} className="bg-white rounded-lg shadow-sm border p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{result.lga}, {result.state}</p>
                        </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-semibold">{result.totalVotes.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                      ...
                      {filteredResults.slice(0, 20).map((result) => (
                        <tr key={result.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{result.pollingUnit}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{result.candidates[0]?.party}</div>
                            <div className="text-sm text-gray-500">{result.candidates[0]?.votes.toLocaleString()} votes</div>
                          </td>
                          ...
                          >
                            <Popup>
                              <div className="p-2 min-w-64">
                                <h4 className="font-semibold text-gray-900 mb-2">{result.pollingUnit}</h4>
                                <p className="text-sm text-gray-600 mb-2">{result.lga}, {result.state}</p>
                                <div className="space-y-1 mb-3">
                                  {result.candidates.slice(0, 3).map((candidate, index) => (
                                    <div key={index} className="flex justify-between text-sm">
                                      <span className="font-medium">{candidate.party}</span>
                                      <span>{candidate.votes.toLocaleString()} ({candidate.percentage.toFixed(1)}%)</span>
                                    </div>
                                  ))}
                                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              ...
              accurate, and real-time election results from polling units across all 36 states and 774 LGAs.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Complete transparency in election reporting with real-time updates and verification processes.
              </p>
            </div>
            <div className="text-center">
              <div className="bg-primary-100 rounded-full p-4 w-16 h-16 mx-auto mb-4">
              ...
              <p className="text-gray-600">
                Rigorous verification processes ensure the highest level of accuracy in reported results.
              </p>
            </div>
            <div className="text-center">
              <div className="bg-primary-100 rounded-full p-4 w-16 h-16 mx-auto mb-4">
              ...
              <p className="text-gray-600">
                Easy access for all citizens to view election results in multiple formats and layouts.
              </p>
            </div>
          </div>
        </div>
      ...
      <footer id="contact" className="bg-gray-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li><a href="#live" className="text-gray-400 hover:text-white">Live Results</a></li>
          ...
          <div className="border-t border-gray-800 mt-8 pt-8 text-center">
            <p className="text-gray-400 text-sm">
              © 2024 eReporter. All rights reserved.
            </p>
          </div>
        </div>
      </footer>
